...
new CommDAO().update(request,response,"guides",ext,true,false); 
HashMap mmm = new CommDAO().getmap(id,"guides"); 
...
<!-- servletformstart 
<form  action="/travelopr/travelopr?ac=guidesxg&id=<%=mmm.get("id")%>"   autocomplete="off"  style="display: inline;font-size: 15px" name="f1" method="post"  onsubmit="return checkform()" >
  servletformend -->
  <!-- s2formstart -->
<form  name="f1" method="post"  onsubmit="return checkform()" action="/travelopr/travelopr/guidesxg.do?id=<%=mmm.get("id")%>" > 
  <!-- s2formend -->
<center>
...
if(gnoobj.value!=""){  
var ajax = new AJAX(); 
ajax.post("/travelopr/factory/checkno.jsp?table=guides&col=gno&value="+gnoobj.value+"&checktype=update&id=<%=mmm.get("id")%>&ttime=<%=Info.getDateStr()%>"); 
//ajax.post(encodeURI("/travelopr/factory/checkno.jsp?table=guides&col=gno&value="+gnoobj.value+"&checktype=update&id=<%=mmm.get("id")%>&ttime=<%=Info.getDateStr()%>")); 
